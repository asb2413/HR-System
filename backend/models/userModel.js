...
userSchema.statics.signup = async function (
  ...
  if (usernameExsits) {
    throw Error("Username Alredy Exsits");
  }

  const phoneExsits = await this.findOne({ phone });
  if (phoneExsits) {
    throw Error("Phone Alredy Exsits");
  }

  const emailExsits = await this.findOne({ email });
  if (emailExsits) {
    throw Error("Email Alredy Exsits");
  }

  const identityExsits = await this.findOne({ identity });
  if (identityExsits) {
    throw Error("identity Alredy Exsits");
  }
